    ...
    <div style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", background: "#FFFFFF", color: "#111111", minHeight: "100vh" }}>
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "40px 24px 64px" }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                ...
                {result.themesHit.map((t) => (
                  <span key={t.id} style={{ fontSize: 13, fontWeight: 600, padding: "6px 10px", border: "1.5px solid", borderColor: t.coef >= 0.2 ? "#1F7A33" : t.coef <= -0.2 ? "#E10600" : "#8A8A8A", color: t.coef >= 0.2 ? "#1F7A33" : t.coef <= -0.2 ? "#E10600" : "#555" }}>
        ...
        <div style={{ marginTop: 48, paddingTop: 14, borderTop: "3px solid #111", fontSize: 12, color: "#8A8A8A", lineHeight: 1.6 }}>
          Two calibrated models from the complete monthly Discover data — every English page with at least one click across all 14 full months, 8,292 stories — now including publish timing: big-hit odds (≥5,000 clicks, AUC 0.81) and entry odds (AUC 0.82). A third of stories enter Discover; only 2.8% become big hits — chase the second number. Animals/science stories carry extra jackpot upside the average understates. No tool replaces news judgement.
        </div>
      </div>
    </div>
  );
